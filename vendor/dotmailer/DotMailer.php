...
require_once 'Exception/AddressBookNotFoundException.php';
require_once 'Exception/InvalidContactTypeException.php';
require_once 'Exception/InvalidDateTimeFormatException.php';
require_once 'Exception/InvalidFileFormatException.php';
require_once 'Exception/InvalidParametersException.php';
require_once 'Exception/MissingRequiredParametersException.php';
require_once 'Exception/UsernameAndPasswordNotFoundException.php';
...
class DotMailer
{
    ...
    public function SendCampaignToContact($campaignId, $contactId, $sendDate)
    {
        if (!is_numeric($campaignId)) {
            ...
            throw new CRM_Core_Exception('Invalid Dotmailer campaign ID given.');
        }

        if (!is_numeric($contactId)) {
            //throw new Exception\InvalidParametersException('Invalid contact ID given.');
            throw new CRM_Core_Exception('Invalid contact ID given.');
        }

        $date_regex = '/(\-)?\d{4}-[0-1][0-9]-[0-3][0-9]T[0-2][0-9]:[0-5][0-9]:[0-5][0-9](Z|(\+|\-)[0-5][0-9]:[0-5][0-9])?/';
        if (preg_match($date_regex, $sendDate) == 0) {
            //throw new Exception\InvalidDateTimeFormatException('startDate is invalid.');
            throw new CRM_Core_Exception('startDate is invalid.');
        }
    ...
    /**
     * List all of the campaigns.
     *
     * @return array A list of campaigns available.
     */
    public function ListCampaigns($select, $skip)
    {
        $parameters = array(
          'username' => $this->username,
          'password' => $this->password,
        ...
        $result = $this->makeSoapCall('ListCampaigns2', $parameters);
        
        if ($result === false) {
            return false;
        }

        if (is_array($result->ListCampaigns2Result->APICampaign)) {
            return $result->ListCampaigns2Result->APICampaign;
        }
        return array($result->ListCampaigns2Result->APICampaign);
    }    
     ...
     * @return array A list of campaigns available.
     */
    public function ContactDataFields()
    {
        $parameters = array(
          'username' => $this->username,
          'password' => $this->password,
        ...
        $result = $this->makeSoapCall('ContactDataFields', $parameters);
